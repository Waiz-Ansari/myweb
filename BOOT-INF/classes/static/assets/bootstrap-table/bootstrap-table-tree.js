            ...
            $.each(this.header.fields, function (j, field) {
                ...
                if (column.checkbox || column.radio) {
                    ...
                    if (that.options.treeView && column.field == that.options.treeField) {
                        var indent = '';
                        for(var i=0;i<item.resourceType;i++){
                            indent = [sprintf('<span style="margin-left: 15px;"></span>'),indent].join('');
                        }
                        ...
                        if(child.length > 0){
                            icon = sprintf('<span class="tree-icon %s" style="cursor: pointer; margin: 0px 5px;"></span>', item.state == "open" ? that.options.expandIcon : that.options.collapseIcon);
                        }else{
                            icon = sprintf('<span class="tree-icon" style="cursor: pointer; margin: 0px 5px;"></span>')
                        }
//                      icon = sprintf('<span class="tree-icon %s" style="cursor: pointer; margin: 0px 5px;"></span>', child.state == "open"&&child.length > 0 ? that.options.expandIcon : that.options.collapseIcon);
//                        icon = sprintf('<span class="tree-icon %s" style="cursor: pointer; margin: 0px 5px;"></span>', item.state == "open"&&child.length > 0 ? that.options.expandIcon : "");
                    }
        ...
        this.$body.find('> tr[data-index] > td > .tree-icon').off('click').on('click', function (e) {
            ...
            data[index].state = icon.hasClass(that.options.expandIcon)?"closed":"open";
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        collapseIcon: "iconfont icon-nodecollapse",//折叠样式
        expandIcon: "iconfont icon-nodeexpand"//展开样式
    });
})(jQuery);
